...
type Response = Either<AppError.UnexpectedError, Result<Seed>>;
...
export class GenerateSeedUseCase implements UseCase<string, Promise<Response>> {
  async execute(diceSequenceEntroy?: string): Promise<Response> {
    const diceEntropyOrError = SeedDiceEntropy.create({
      value: diceSequenceEntroy,
    });
    const dtoResult = Result.combine([diceEntropyOrError]);
