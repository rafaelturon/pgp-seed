...
export class SeedDiceEntropy extends ValueObject<IDiceEntropyProps> {
    public static minLength: number = 32;
    ...
    public static create (props: IDiceEntropyProps): Result<SeedDiceEntropy> {
        const propsResult = Guard.againstNullOrUndefined(props.value, "diceEntropy");

        if (!propsResult.succeeded) {
            return Result.fail<SeedDiceEntropy>(propsResult.message);
        } else {
            if (!this.isAppropriateLength(props.value)) {
                return Result.fail<SeedDiceEntropy>('Seed dice entropy does not meet criteria [32 chars min].');
            }
            return Result.ok<SeedDiceEntropy>(new SeedDiceEntropy({
                value: props.value
            }));
        }
    }
